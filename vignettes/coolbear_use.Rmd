...
where N is the final number of germinating seeds and n~j~ and n~i~ are the cumulative number of seeds germinated by adjacent counts at times t~j~ and t~i~ , respectively, when n~i~ \< N/2 \< n~j~. In essence, this formula identifies the midpoint of the cumulative germination sigmoidal curve, where the slope is steepest.
...
If T~50~ can't be calculated (e.g. not enough seeds germinated) the function will output NA. 'coolbear()' can efficiently be used within a loop to calculate T~50~ of several experiments at a time. Given the following germination experiment results (including a row with final viability):
